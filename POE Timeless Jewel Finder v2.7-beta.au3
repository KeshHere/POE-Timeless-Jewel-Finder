...
Local $hGUI = GUICreate("Timeless Jewel Finder-v2.7-beta",770,530) ;gui create
GUISetFont(10)
	...
	Switch GUIGetMsg()
		...
		Case $trade
			...
			if GUICtrlRead($JewelCombo) == '' Then
				...
				if $nums <> '' Then
					ShellExecute('https://www.pathofexile.com/trade/search/Crucible?q='&$tradelink)
;~ 					ShellExecute('https://www.pathofexile.com/trade/search/Hardcore%20Kalandra?q='&$tradelink)
		...
		Case $nearcombo
			...
			if GUICtrlRead($nearcombo) == 'near: Zealots Oath' Then GUICtrlSetData($PassiveList, $nearZealotsOath)
